...
static class ReservationManagement
{
    ...
    {
        string[] options = { "Table Selection", "Guest Count", "Date & Time", "Dish Selection", "Cancel Reservation", "Back" };
        int selectedIndex = 0;
        bool managing = true;
    ...
    private static void EditTableSelection(ReservationModel reservation)
    {
        ...
        if (selectedTable != null && selectedTable.ID != reservation.TableId)
        {
    ...
    private static void EditGuestCount(ReservationModel reservation)
    {
        ...
        {
            _reservationsLogic.UpdateGuestCountForReservation(newCount, reservation);
